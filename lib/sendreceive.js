...
BoxReceiver.prototype.dataReceived = function dataReceived (data) {
    ...
    while (self._unprocessed.length > offset + 2) {
        ...
        if (length === 0 || self._unprocessed.length < nextOffset) {
            // either the box is done, or there is not enough data in the
            // buffer and we have to wait for more data
            return;
        }
